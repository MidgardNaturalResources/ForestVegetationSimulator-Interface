...
# FINAL operational model: RD + origin (BYI dropped, not significant)
m_final <- glm(ing_tph ~ RD + planted, family=quasipoisson(link="log"), data=d)
cat("\n=== FINAL ingrowth model: RD + origin ===\n"); print(round(summary(m_final)$coef,4))
cat(" plantation rate ratio:",round(exp(coef(m_final)["planted"]),3),"(<1 = less ingrowth)\n")
cof<-coef(m_final); con<-file("/sessions/elegant-lucid-cannon/mnt/outputs/koa_harness/results/ingrowth_final_coef.csv","w")
cat("term,estimate\n",file=con); for(t in names(cof)) cat(sprintf("%s,%.8f\n",t,cof[t]),file=con); close(con)
cat("\nWrote koa_ingrowth_byi_obs.csv, ingrowth_byi_coef.csv, ingrowth_final_coef.csv\n")
