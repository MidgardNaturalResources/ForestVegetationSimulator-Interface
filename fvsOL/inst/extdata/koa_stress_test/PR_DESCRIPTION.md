...
### Proposed fix (tuned on the data, validated to 300 yr)
- `koa_survival_calibrated.R`: annual mortality = base(origin) + a linear self-thinning lift that starts at 0.65 relative density (SDI/SDImax), rises to 0.15/yr at 0.85 RD, and plateaus above. Base 0.3%/yr natural, 0.6%/yr plantation (low-density background; plantations higher from establishment loss, natural stands carry the self-thinning load). BYI is deliberately NOT a direct mortality term: the data show its apparent effect is one cluster of high-BYI natural plots with no plantations, so it cannot be estimated for plantations; BYI instead shapes density through growth.
...
- Stands become realistically multi-cohort (sustained BA/density), no runaway/collapse across origin x BYI. Over long projections ingrowth starts high, declines as the canopy closes, and settles into turnover balance with mortality (~17.5/yr natural, ~5/yr plantation), giving a stable self-replacing stand.
...
### Integrated 300-yr validation
- 30 adversarial scenarios (5 extreme starts x origin x BYI) of the full system (survival + size allocation + ingrowth): all robust (no NaN, no collapse, bounded BA, monotone QMD), 100% reach steady state (SDI 69-82% of SDImax, BA 21-26). Overstocked inputs (to 149% SDImax) self-correct to the self-thinning line within 3-5 yr.
- All R drop-in constants verified to match the fitted CSVs (`verify_consistency.py`).

Note: the harness is the equation-level engine with a machine-precision fidelity check against HiGy.R (not the compiled FVS binary); MAI culmination ages run young; both should be confirmed in true FVS.
